...
class CommandParserNode(Node):
    def __init__(self):
        ...
        self.latest_objects = []  # VSS 최신 인식 결과 저장
        self.get_logger().info('CommandParser 준비 완료 (Gemini + VSS 모드)')

    def on_objects(self, msg: String):
        """VSS 인식 결과 수신 — 최신 상태 저장"""
        try:
    ...
    def on_command(self, msg: String):
        ...
        if self.latest_objects:
            ...
            scene_info = "장면 정보 없음 (VSS 미연결)"

        prompt = f"""당신은 로봇 팔 제어 시스템입니다.
아래 JSON만 출력하세요. 다른 텍스트, 마크다운 절대 금지.
{{"target_label": "물체이름(영어 소문자)", "action": "pick", "position": {{"x": 0.0, "y": 0.0, "z": 0.0}}}}

현재 장면의 물체: {scene_info}
명령: {user_text}

장면에 있는 물체 중에서만 선택하세요. position은 center_3d 값을 그대로 사용하세요."""
        ...
        try:
            ...
            self.get_logger().error(f'JSON 파싱 실패: {e} / 원문: {resp.text}')
